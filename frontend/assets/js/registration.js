...
/* Save user to server - use USER_SERVICE_URL from user_sync.js */
const saveUserToServer = async (user) => {
    const USER_SERVICE_URL = 'http://localhost:5007';
    try {
        const {isLoggedIn, ...userDataForServer} = user;
        ...
        if (!response.ok) {
            console.warn('Failed to save user to server');
            return false;
        }

        console.log('User saved to server:', user.cardNumber);
        return true;
    } catch (error) {
...
window.clearSignInForm = function () {
